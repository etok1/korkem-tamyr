...
const media = [
    {
        name: 'instagram link',
        link: '', 
        icon: <Instagram width={30} height={30} className='md:h-[20px] md:w-[20px]'  color='#5966B1'/>,
    },
     {
        name: 'youtube link',
        link: '', 
        icon: <Youtube width={30} height={30} className='md:h-[20px] md:w-[20px] lg:h-[20px] lg:w-[20px]' color='#5966B1'/>,
    }
]
...
export default function MobileHeader() {
  ...
  return (
   <header className='lg:hidden flex h-auto px-10 py-5 bg-headerWhite relative'  style={{ fontFamily: 'Montserrat, sans-serif'}}>
        <div className="flex items-center justify-between w-full">
           <NavLink to='/'><img src={logo} alt="logo" width={100} className='w-[70px]'/></NavLink>
            <button onClick={handleOpen} aria-label="Toggle menu">{isOpen ? <X color='#5966B1' width={30} height={30}/>  : <Menu color='#5966B1' />}</button>
        </div>
