        ...
        <footer className="mt-auto py-4 bg-blueColor w-full h-[450px] relative bottom-0 left-0 right-0"  style={{  fontFamily: "Shantell Sans, cursive" }}>
            <div className="max-w-[1200px] w-full h-full m-auto  flex flex-col items-center justify-between">
                <div style={{ fontFamily: 'Montserrat Alternates, sans-serif'}}>
                    ...
                    <p className="text-xs lg:text-[15px] mt-0 lg:tracking-[0.15em] text-[#93AECA75] font-medium">{language === 'en' ? 
                    'Korkem Tamyr is in the heart of every child.' :
                    'Көркем тамыр — әр бала жүрегінде'
                    }</p>
                </div>
                <div className="flex flex-col sm:flex-row items-start justify-between w-full xs:w-[90%] text-left px-3">
                    <div className="text-dirtBlue">
                        <h3 className="text-white text-sm xs:text-base md:text-xl lg:text-2xl font-semibold tracking-[0.1em]" style={{ fontFamily: 'Montserrat Alternates, sans-serif'}}>Contacts:</h3>
                        <p className="group"><a className="text-xs xs:text-sm md:text-base  flex items-center gap-1 xs:gap-2 mt-2 group-hover:text-white transition-all duration-150" href='tel:+77001115555'><Phone color="#8C93C7"  className="w-[16px] h-[16px] xs:w-[20px] xs:h-[20px] group-hover:stroke-white transition-stroke duration-150"/>+7 700 111 5555</a></p>
                        <p className="group"><a className="text-xs xs:text-sm md:text-base  flex items-center gap-1 xs:gap-2  mt-1 group-hover:text-white transition-all duration-150" href='mailto:korkemtamyr@gmail.com'><Mail color="#8C93C7"  className="w-[16px] h-[16px] xs:w-[20px] xs:h-[20px] group-hover:stroke-white transition-stroke duration-150"/>korkemtamyr@gmail.com</a></p>
                    </div>
